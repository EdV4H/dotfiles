...
function cli:get_all_packages()
  ...
  local function generate_node(tbl, list)
    ...
    if tbl.dependencies then
      for _, v in pairs(tbl.dependencies) do
        ...
        list[v[1]].type = v.dev and 'Loacl Plugin' or 'Remote Plugin'
  ...
  local list = {}
  for _, data in pairs(pkg.repos or {}) do
...
function cli:boot_strap()
  ...
  if helper.isdir(self.lazy_dir) then
    helper.success('Found lazy.nvim skip download')
    return
  end
...
function cli:installer(type)
  ...
  for name, v in pairs(packages or {}) do
    if v.type:find('Remote') then
      ...
      if helper.isdir(path) and type == 'install' then
        helper.purple('\t⚡️ Skip already in plugin ' .. name)
      else
        ...
        table.insert(res, filed)
      end
    else
      helper.purple('\t⚡️ Skip local plugin ' .. name)
    end
  end
  ...
  helper.error('Some plugins not install or update success please run install again.')
end
...
function cli.doctor(pkg_name)
  local list = cli:get_all_packages()
  if not list then
    return
  end

  helper.yellow('🚑️ Total: ' .. vim.tbl_count(list) + 1 .. ' Plugins')
  local pkgs = pkg_name and { [pkg_name] = list[pkg_name] } or list
